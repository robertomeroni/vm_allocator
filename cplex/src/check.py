...
def check_overload(vms, pms, time_step):
    ...
    for pm_id, pm in pms.items():
        if cpu_load[pm_id] > 1 or memory_load[pm_id] > 1:
            ...
            for vm in vms.values():
                if vm['allocation']['pm'] == pm_id or vm['run']['pm'] == pm_id or vm['migration']['from_pm'] == pm_id or vm['migration']['to_pm'] == pm_id:
            ...
            if effective_cpu_load > pm['capacity']['cpu'] or effective_memory_load > pm['capacity']['memory']:
                raise ValueError(f"PM {pm_id} is overloaded: cpu_load {cpu_load[pm_id]}, memory_load {memory_load[pm_id]}, effective_cpu_load {effective_cpu_load}, effective_memory_load {effective_memory_load}.")
    ...
    for pm_id, pm in pms.items():
        if pm['s']['load']['cpu'] == 0 and pm['s']['load']['memory'] == 0:
            if pm['s']['state'] == 1 and pm['s']['time_to_turn_on'] == 0:
                for vm in vms.values():
                    if vm['allocation']['pm'] == pm_id or vm['run']['pm'] == pm_id or vm['migration']['from_pm'] == pm_id or vm['migration']['to_pm'] == pm_id:
                        raise ValueError(f"VM {vm['id']} is allocated to PM {pm_id} with zero load: {pm['s']['load']}.")
