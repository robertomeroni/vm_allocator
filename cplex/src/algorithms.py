...
def manage_pms_allocation(pms, allocation):
    """
    Manage the allocation status of physical machines (PMs).

    Args:
        pms (dict): Dictionary of physical machines with their states and loads.
        allocation (list): List of allocation dictionaries containing 'pm_id'.

    Returns:
        dict: A dictionary mapping each pm_id to its 'on' status (1 for on, 0 for off).
    """
    ...
    for pm_id, pm in pms.items():
        if pm['s']['state'] == 1 and pm['s']['time_to_turn_on'] > 0:
            ...
            for vm in vms.values():
                if (vm['allocation']['pm'] == pm_id or
                    vm['run']['pm'] == pm_id or
                    vm['migration']['from_pm'] == pm_id or
                    vm['migration']['to_pm'] == pm_id):
                    is_on[pm_id] = 1
                    break
    return is_on
...
def first_fit(vms, pms):
    ...
    sorted_pms = sorted(pms.values(), key=lambda pm: (
...
def worst_fit(vms, pms):
    ...
    sorted_pms = sorted(pms.values(), key=lambda pm: (
        ...
        pm['capacity']['memory'] - pm['s']['load']['memory'] * pm['capacity']['memory']
    ), reverse=True)
...
def shi_migration(vms, pms, failed_migrations_limit=None):
    ...
    for pm_id, pm in pms.items():
        magnitude_pm[pm_id] = get_magnitude_pm(pm, vms)
    sorted_pms = sorted(pms.values(), key=lambda pm: magnitude_pm[pm['id']], reverse=True)
    ...
    for vm_id, vm in vms.items():
        ...
        for pm in sorted_pms:
            if vm_fits_on_pm(vm, pm):
                ...
                if PRINT_TO_CONSOLE:
                    print(f"VM {vm_id} allocated to PM {pm['id']}")
                pm['s']['load']['cpu'] += vm['requested']['cpu'] / pm['capacity']['cpu']
                pm['s']['load']['memory'] += vm['requested']['memory'] / pm['capacity']['memory']
                magnitude_pm[pm['id']] = get_magnitude_pm(pm, vms)
                break

    allocation = [{'vm_id': vm_id, 'pm_id': vm['allocation']['pm']} for vm_id, vm in vms.items()]
...
def guazzone_bfd(vms, pms, idle_power):
    allocation = {vm_id: {'vm_id': vm_id, 'pm_id': None} for vm_id in vms}
    sorted_vms = sorted(vms.values(), key=lambda vm: (
        vm['requested']['cpu'],
        vm['requested']['memory'],
        -vm['id']
    ), reverse=True)
