...
from config_azure import (
    ...
    allocation_time_range,
    default_num_physical_machines,
    default_num_virtual_machines,
    state_percentage,
    running_percentage,
    migration_time_memory_dirty_rate,
    ...
    for i in range(n):
        ...
        for j in range(n):
            if i == j:
                ...
                latencies.append(
                    round(np.random.uniform(latency_range[0], latency_range[1]), 2)
                )
        latency_matrix.append(latencies)
    return latency_matrix


def generate_physical_machines(
    n,
    cpu_capacity,
    memory_capacity,
    ...
    for i in range(n):
        ...
        state = (
            ...
            round(
                np.random.uniform(time_to_turn_off_range[0], time_to_turn_off_range[1]),
                1,
            ),
...
def generate_virtual_machines(
    ...
    virtual_machines = []
    on_pms = [
        pm for pm in physical_machines if pm[3][3] == 1
    ]  # List of ON physical machines
    num_running = int(n * running_percentage / 100)

    for i in range(n):
        id = i
        requested = (
            np.random.choice(requested_cpu),
            np.random.choice(requested_memory),
        )
        ...
        if i < num_running:
            suitable_pms = [
                pm
                for pm in on_pms
                if pm[1][0] >= requested[0] and pm[1][1] >= requested[1]
            ]
            if suitable_pms:
                allocated_pm = np.random.choice(suitable_pms)[0]
                total_execution_time = round(
                    np.random.uniform(execution_time_range[0], execution_time_range[1]),
                    1,
                )
                current_execution_time = round(
                    np.random.uniform(0, total_execution_time), 1
                )
                ...
                run = (
                    ...
                    round(
                        np.random.uniform(
                            execution_time_range[0], execution_time_range[1]
                        ),
                        1,
                    ),
                    -1,
            ...
            total_execution_time = round(
                np.random.uniform(execution_time_range[0], execution_time_range[1]), 1
            )
            run = (0.0, total_execution_time, -1)

        allocation_time = round(
            np.random.uniform(allocation_time_range[0], allocation_time_range[1]), 1
        )
...
def update_physical_machine_loads(physical_machines, virtual_machines):
    ...
    pm_loads = {
        pm[0]: {"cpu_load": 0.0, "memory_load": 0.0} for pm in physical_machines
    }
...
parser = argparse.ArgumentParser(
    description="Generate data for physical and virtual machines."
)
parser.add_argument(
    "--terminal", action="store_true", help="Use terminal input for number of machines"
)
parser.add_argument(
    "--simulation",
    nargs=2,
    metavar=("NUM_PHYSICAL_MACHINES", "OUTPUT_FILE"),
    help="Run simulation with specified number of physical machines and output file",
)
parser.add_argument(
    "--scalability_test",
    nargs=2,
    metavar=("NUM_PHYSICAL_MACHINES", "NUM_VIRTUAL_MACHINES"),
    help="Run scalability test with specified number of physical and virtual machines",
)
args = parser.parse_args()

if args.terminal:
    num_physical_machines = get_terminal_input(
        "Number of physical machines", default_num_physical_machines
    )
    num_virtual_machines = get_terminal_input(
        "Number of virtual machines", default_num_virtual_machines
    )
    output_folder = data_folder_path
elif args.simulation:
    num_physical_machines = (
        int(args.simulation[0]) if args.simulation[0] else default_num_physical_machines
    )
    num_virtual_machines = 0
    output_file = args.simulation[1]
    output_folder = os.path.dirname(
        output_file
    )  # Use the directory of the provided file path
elif args.scalability_test:
    num_physical_machines = (
        int(args.scalability_test[0])
        if args.scalability_test[0]
        else default_num_physical_machines
    )
    num_virtual_machines = (
        int(args.scalability_test[1])
        if args.scalability_test[1]
        else default_num_virtual_machines
    )
...
physical_machines = generate_physical_machines(
    num_physical_machines,
    pm_cpu_capacity,
    pm_memory_capacity,
    pm_time_to_turn_on_range,
...
virtual_machines = generate_virtual_machines(
    num_virtual_machines,
    num_physical_machines,
    vm_requested_cpu,
    vm_requested_memory,
